...
export default async function CartPage({
  ...
  if (!customer) redirect("/account/login?next=" + encodeURIComponent("/account/cart"));

  const { added, checkoutError } = await searchParams;
  const { items, subtotal, taxesAndFees, total } = await getCartSummary(customer.id);

  return (
    <>
      <PageHero image="/images/hero-rooms.jpg" eyebrow="Your Trip" title="Cart" />
        ...
        <div className="mx-auto max-w-4xl">
          ...
          {items.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-sm text-charcoal-700">Your cart is empty.</p>
            ...
            <>
                    ...
                    <div className="flex items-center gap-5">
                      <form action={updateCartItemAction.bind(null, item.id)} className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="w-16 border-b border-charcoal-900/20 py-1 text-sm bg-transparent focus:outline-none focus:border-gold-500"
                        />
                        <button
                          type="submit"
                      ...
                      <p className="font-serif text-lg text-charcoal-900 w-20 text-right">
                        ${item.lineTotal}
                      </p>
              ...
              <div className="mt-10 bg-charcoal-950 text-ivory-50 p-8">
